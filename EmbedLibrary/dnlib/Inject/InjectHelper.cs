...
namespace dnlib.Inject
{
    public static class InjectHelper
    {
        ...
        private static TypeDef PopulateContext(TypeDef typeDef, InjectContext ctx)
        {
            TypeDef ret;
            IDnlibDef existing;
            if (!ctx.Maps.TryGetValue(typeDef, out existing))
            {
                ret = Clone(typeDef);
                ctx.Maps[typeDef] = ret;
            }
            else
            ...
            foreach (MethodDef method in typeDef.Methods)
                ret.Methods.Add((MethodDef)(ctx.Maps[method] = Clone(method)));

            foreach (FieldDef field in typeDef.Fields)
                ret.Fields.Add((FieldDef)(ctx.Maps[field] = Clone(field)));
        ...
        {
            var newTypeDef = (TypeDef)ctx.Maps[typeDef];
        ...
        {
            var newMethodDef = (MethodDef)ctx.Maps[methodDef];
        ...
        {
            var newFieldDef = (FieldDef)ctx.Maps[fieldDef];
        ...
        public static TypeDef Inject(TypeDef typeDef, ModuleDef target)
        {
            ...
            return (TypeDef)ctx.Maps[typeDef];
        }
        ...
        public static MethodDef Inject(MethodDef methodDef, ModuleDef target)
        {
            ...
            ctx.Maps[methodDef] = Clone(methodDef);
            CopyMethodDef(methodDef, ctx);
            return (MethodDef)ctx.Maps[methodDef];
        }
        ...
        {
            var ctx = new InjectContext(typeDef.Module, target);
            ctx.Maps[typeDef] = newType;
            PopulateContext(typeDef, ctx);
            Copy(typeDef, ctx, false);
            return ctx.Maps.Values.Except(new[] { newType });
        }
    }
}
